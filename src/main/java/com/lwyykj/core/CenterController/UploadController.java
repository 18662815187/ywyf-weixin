...
import com.lwyykj.core.upservice.UploadService;
...
public class UploadController {
	...
	@ResponseBody
	public List<String> uploadPics(@RequestParam(required = false) MultipartFile[] pics, HttpServletRequest request,
	...
	@ResponseBody
	private void fildUpload1(MultipartFile[] file, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		// 获得物理路径webapp所在路径
		File dir = null;
		Calendar cal = Calendar.getInstance();
		...
		// "/" + time);
		// 此处使用的是绝对地址，此绝对地址是tomcat图片服务器的虚拟路径映射
		String superPath = "D://upFiles/" + "upload" + "/" + time;
		// System.out.println(realpath+"*********************");
		dir = new File(superPath);
		if (!dir.exists()) {
			dir.mkdirs();
		}
		// String pathRoot =
		...
		for (MultipartFile mf : file) {
			if (!mf.isEmpty()) {
				...
				mf.transferTo(new File(superPath + "/" + newName));
				listImagePath.add(path);
				if ("" != path1) {
					path1 = path1 + "," + path;
				} else {
	...
	// 图集单图都可以
	@RequestMapping(value = "/uploadXX2", method = RequestMethod.POST)
	@ResponseBody
	private Object fildUpload2(MultipartFile[] file1, HttpServletRequest request) throws Exception {
		// @RequestParam(value = "file", required = false)
		// Map<String, Object> map = new HashMap<>();
		// 获得物理路径webapp所在路径
		File dir = null;
